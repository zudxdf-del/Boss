...
function reset(){projectiles.length=0;warnings.length=0;arena.reset(renderer.w,renderer.h);player.reset(renderer.w,renderer.h,arena);boss.reset(renderer.w,renderer.h,arena);camera.x=player.x;camera.y=player.y;ended=false;running=true;menu.classList.add('hidden');start.textContent='НАЧАТЬ БОЙ';last=performance.now();}
start.addEventListener('click',reset);
addEventListener('resize',()=>{arena.reset(renderer.w,renderer.h);camera.resize(renderer.w,renderer.h);if(!running){player.reset(renderer.w,renderer.h,arena);boss.reset(renderer.w,renderer.h,arena);camera.x=player.x;camera.y=player.y;}});
...
function playerFire(){if(player.cooldown>0)return;let target;if(input.isTouchDevice)target=autoAim();else target=camera.toWorld(input.getAim(player.x,player.y,renderer.w,renderer.h).x,input.getAim(player.x,player.y,renderer.w,renderer.h).y);const dx=target.x-player.x,dy=target.y-player.y,d=Math.hypot(dx,dy)||1;projectiles.push(new Projectile(player.x,player.y,dx/d*720,dy/d*720,'player',player.damage));player.cooldown=player.fireRate;}
...
arena.reset(renderer.w,renderer.h);player.reset(renderer.w,renderer.h,arena);boss.reset(renderer.w,renderer.h,arena);camera.resize(renderer.w,renderer.h);camera.x=player.x;camera.y=player.y;requestAnimationFrame(loop);
