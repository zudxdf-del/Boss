...
function makeBoss(){const add=(id,x,y,kind,hp)=>({id,x,y,kind,hp,maxHp:hp,alive:true,shotTimer:35+Math.floor(Math.random()*25)});return{x:500,y:300,angle:0,hp:0,maxHp:0,modules:[add('core',0,0,'core',120),add('top',0,-86,'armor',65),add('bottom',0,86,'armor',65),add('left',-86,0,'cannon',75),add('right',86,0,'cannon',75),add('tl',-62,-62,'cannon',60),add('tr',62,-62,'cannon',60),add('bl',-62,62,'armor',60),add('br',62,62,'armor',60)]};}
function syncBoss(b){b.maxHp=b.modules.reduce((n,m)=>n+m.maxHp,0);b.hp=b.modules.reduce((n,m)=>n+Math.max(0,m.hp),0)}
function createRoom(){const r={code:makeCode(),players:[],phase:'waiting',bullets:[],boss:makeBoss()};syncBoss(r.boss);rooms.set(r.code,r);return r}
...
function inside(x,y){return Math.hypot(x-500,y-500)<=405}
...
function bossFire(r,m){const live=r.players.filter(p=>p.hp>0);if(!live.length||!m.alive||m.kind!=='cannon')return;const t=live[Math.floor(Math.random()*live.length)],sx=r.boss.x+m.x,sy=r.boss.y+m.y,dx=t.x-sx,dy=t.y-sy,l=Math.hypot(dx,dy)||1;r.bullets.push({id:id(),owner:'boss:'+m.id,x:sx,y:sy,vx:dx/l*4.8,vy:dy/l*4.8,life:190,enemy:true,module:m.id})}
...
setInterval(()=>{for(const r of rooms.values()){if(r.phase!=='playing')continue;r.boss.angle+=.025;r.boss.x=500+Math.cos(r.boss.angle)*120;r.boss.y=300+Math.sin(r.boss.angle)*45;for(const m of r.boss.modules){if(m.alive&&m.kind==='cannon'){m.shotTimer--;if(m.shotTimer<=0){bossFire(r,m);m.shotTimer=55+Math.floor(Math.random()*30)}}}for(const p of r.players){p.x+=p.input.x*7;p.y+=p.input.y*7;const dx=p.x-500,dy=p.y-500,d=Math.hypot(dx,dy),max=390;if(d>max){const k=max/d;p.x=500+dx*k;p.y=500+dy*k}if(p.cooldown>0)p.cooldown--}const next=[];for(const b of r.bullets){b.x+=b.vx;b.y+=b.vy;b.life--;if(b.life<=0||!inside(b.x,b.y))continue;if(!b.enemy&&hitModule(r,b))continue;if(b.enemy){const p=r.players.find(p=>p.hp>0&&Math.hypot(b.x-p.x,b.y-p.y)<23);if(p){p.hp=Math.max(0,p.hp-10);continue}}next.push(b)}r.bullets=next;syncBoss(r.boss);const core=r.boss.modules.find(m=>m.id==='core');if(!core.alive)r.phase='won';else if(r.players.length&&r.players.every(p=>p.hp<=0))r.phase='lost';broadcast(r,snapshot(r))}},1000/TICK);

const server=http.createServer(async(req,res)=>{
 if(req.url==='/health'){res.writeHead(200,{'content-type':'application/json; charset=utf-8','cache-control':'no-store'});return res.end(JSON.stringify({ok:true,rooms:rooms.size,websocket:true,port:PORT}))}
 if(req.url==='/'||req.url==='/index.html'){
  try{
   let html=await readFile(new URL('./public/index.html',import.meta.url),'utf8');
   const draw=`function draw(){const scale=canvas.width/world.width;ctx.setTransform(scale,0,0,scale,0,0);ctx.clearRect(0,0,world.width,world.height);ctx.fillStyle='#02040b';ctx.fillRect(0,0,1000,1000);ctx.save();ctx.beginPath();ctx.arc(500,500,430,0,Math.PI*2);ctx.clip();const g=ctx.createRadialGradient(500,500,30,500,500,430);g.addColorStop(0,'#151f42');g.addColorStop(1,'#040714');ctx.fillStyle=g;ctx.fillRect(70,70,860,860);ctx.strokeStyle='#263a6b';ctx.lineWidth=2;ctx.beginPath();ctx.arc(500,500,300,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.arc(500,500,185,0,Math.PI*2);ctx.stroke();ctx.restore();const b=state.boss,t=performance.now();ctx.save();ctx.translate(b.x,b.y);ctx.rotate(t/1800);for(const m of b.modules||[]){const s=m.id==='core'?58:50;ctx.save();ctx.translate(m.x,m.y);ctx.fillStyle=!m.alive?'#111722':m.kind==='core'?'#d92f4f':m.kind==='cannon'?'#d88716':'#405878';ctx.strokeStyle=!m.alive?'#293247':m.kind==='core'?'#ffd9df':'#b6c6e8';ctx.lineWidth=5;ctx.fillRect(-s/2,-s/2,s,s);ctx.strokeRect(-s/2,-s/2,s,s);if(m.kind==='core'&&m.alive){ctx.fillStyle='#fff';ctx.fillRect(-10,-10,20,20)}if(m.kind==='cannon'&&m.alive){ctx.fillStyle='#151a25';ctx.fillRect(-7,-28,14,28);ctx.fillStyle='#ffe98a';ctx.fillRect(-4,-35,8,8)}if(m.alive&&m.hp<m.maxHp){ctx.fillStyle='#080b12';ctx.fillRect(-s/2,-s/2-10,s,5);ctx.fillStyle='#51e38a';ctx.fillRect(-s/2,-s/2-10,s*(m.hp/m.maxHp),5)}ctx.restore()}ctx.restore();for(const p of state.players){ctx.save();ctx.translate(p.x,p.y);ctx.fillStyle=p.color;ctx.beginPath();ctx.moveTo(0,-17);ctx.lineTo(13,12);ctx.lineTo(0,7);ctx.lineTo(-13,12);ctx.closePath();ctx.fill();ctx.strokeStyle='#fff';ctx.stroke();ctx.restore()}for(const b of state.bullets){ctx.fillStyle=b.enemy?'#ff4d7d':'#fff';ctx.beginPath();ctx.arc(b.x,b.y,b.enemy?5:4,0,Math.PI*2);ctx.fill()}requestAnimationFrame(draw)}`;
   html=html.replace(/function draw\(\)\{[\s\S]*?requestAnimationFrame\(draw\)/,draw);
   const autoConnect=`<script>window.addEventListener('load',()=>{if(typeof connect==='function'){setTimeout(()=>connect(null),0)}});</script>`;
   html=html.replace('</body>',autoConnect+'</body>');
   res.writeHead(200,{'content-type':'text/html; charset=utf-8','cache-control':'no-store, no-cache, must-revalidate'});return res.end(html)
  }catch(e){console.error(e);res.writeHead(500);return res.end('Client unavailable')}
 }
...
wss.on('connection',ws=>{
 const p={id:id(),ws,room:null,name:'Pilot',x:500,y:800,hp:100,score:0,color:'#63d8ff',cooldown:0,input:{x:0,y:0}};
 send(ws,{type:'hello',id:p.id,maxPlayers:MAX_PLAYERS,world:WORLD});
 ws.on('message',raw=>{let m;try{m=JSON.parse(raw.toString())}catch{return}if(m.type==='ping'){send(ws,{type:'pong',t:Number(m.t)||Date.now()});return}if(m.type==='create'){if(!p.room)join(createRoom(),p,m.name);return}if(m.type==='join'){if(p.room)return;const r=rooms.get(String(m.code||'').toUpperCase());if(!r)return send(ws,{type:'error',message:'Комната не найдена'});if(r.players.length>=MAX_PLAYERS)return send(ws,{type:'error',message:'Комната заполнена'});if(r.phase!=='waiting')return send(ws,{type:'error',message:'Игра уже началась'});join(r,p,m.name);return}if(!p.room)return;if(m.type==='input'){p.input.x=clamp(Number(m.x)||0,-1,1);p.input.y=clamp(Number(m.y)||0,-1,1)}else if(m.type==='fire')fire(p.room,p);else if(m.type==='start'&&p.room.players[0]===p&&p.room.phase==='waiting'){reset(p.room);broadcast(p.room,roomInfo(p.room))}else if(m.type==='restart'&&p.room.players[0]===p&&p.room.phase!=='playing'){reset(p.room);broadcast(p.room,roomInfo(p.room))}});
 ws.on('close',()=>{if(!p.room)return;const r=p.room;r.players=r.players.filter(x=>x!==p);if(!r.players.length)rooms.delete(r.code);else broadcast(r,roomInfo(r))});
});
...
server.listen(PORT,'0.0.0.0',()=>console.log(`Boss server listening on 0.0.0.0:${PORT}`));
